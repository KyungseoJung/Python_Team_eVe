...
@app.route('/')
def index():
    return render_template('index_mq_ver4_LatLng.html', messages=messages)  # 수정된 부분: index.html로 메시지들을 함께 전달
...
def consume_messages():
    ...
    channel.queue_declare(queue='location_queue')   #//#7 수정된 부분: queue 이름 변경
    channel.basic_consume(queue='location_queue', on_message_callback=callback, auto_ack=True)  #//#7 수정된 부분: queue 이름 변경
