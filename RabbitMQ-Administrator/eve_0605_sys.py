...
class two_phase_heuristic:
    def __init__(self, datafile, battery_json_path, truck_csv_path, drawroute_json_path, solution_csv_path,  M=100000, lamb=50000, TN=4, CT=2, num_cpus=4):
        ...
        self.solution_csv_path = solution_csv_path
        self.M = M
        self.lamb = lamb
        self.TN = TN
        self.CT = CT
    ...
    def read_datafile(self):
        ...
        for line in lines:
            ...
            Car_num.append(str(data[6]))
            Detail_add.append(str(data[7]))
        ...
        locations = [[xd_coord, yd_coord]] + coordinates
    ...
    def check_time_window(self, node1, node2):
        travel_time = self.T_ij[(node1, node2)] 
        arrive_time = self.E_i[self.N.index(node1)] + self.S_i[self.N.index(node1)] + travel_time
        end_window = self.L_i[self.N.index(node2)] - self.S_i[self.N.index(node2)]
        return arrive_time <= end_window   
    
    def calculate_k_shortest_paths(self, source, target, k):
        try:
            k_shortest_paths = []
            for path in nx.shortest_simple_paths(self.G, source=source, target=target, weight='weight'):
    ...
    def ORTools_cvrptw(self):
        ...
        def time_callback(from_index, to_index):
            ...
            try:
                return self.T_ij_ot[(str(from_node), str(to_node))] + charge_time + self.S_i * 2
            except KeyError:
                return charge_time 
    ...
    def simulated_annealing(self, time_limit, initial_temperature=100, cooling_rate=0.95, max_iterations=1000):
        ...
        while temperature > 0.1 and iterations < max_iterations:
            ...
            if len(current_route) > 1:
                ...
                if new_energy is not None:
                    if new_energy > current_energy:  # new_energy가 현재 에너지보다 크면 갱신
                        current_route = copy.deepcopy(new_route)
                        current_battery_route = copy.deepcopy(new_battery_route)
                        current_battery_used_capacity = copy.deepcopy(new_battery_used_capacity)
                        current_energy = new_energy
                        ...
                        accept_probability = math.exp((new_energy - current_energy) / temperature)
    ...
    def expand_node_pairs(self, paths):
        ...
        for path in paths:
            ...
            for i in range(len(path_list) - 1):
                ...
                self.expanded_pairs.add((str(path_list[i+1]), 'h' + str(path_list[i+1]))) 
    ...
    def process_route(self, route, battery_route):
        ...
        for i, j in permutations(route, 2):
            if self.G.has_edge(str(i), str(j)):  # 길이 있다면 조합 생성
                k_sp = self.calculate_k_shortest_paths(source=str(i), target=str(j), k=3)
                for path in k_sp:
                    self.k_shortest_paths.add(tuple(path))
        print('**k_shortest_paths: ', self.k_shortest_paths)
    ...
    def save_solution(self, objective_value, variable_names, variable_values):
        try:
            ...
            # truck csv
            headers = ['driver_id', 'client_id', 'delivery_type', 'latitude', 'longitude', 'time' ,'battery_id', 'car_num' , 'detail_address']
                ...
                for truck, stops in Truck_routes.items():
                    ...
                    for stop in stops:
                        if stop not in ['0', str(len(self.N)+1)]:
                            ...
                            car_num = self.Car_num[int(client_id)-1]
                            deatil_add = self.Detail_add[int(client_id)-1]
                            writer.writerow([driver_id, client_id, delivery_type, latitude, longitude, time , battery_id, car_num, deatil_add ])
            ...
            print(f"데이터가 {drawroute_json_filename} 파일에 저장되었습니다.")
            ...
            for key, value in z_values.items():
                if value == 1.0:
                    ...
                    if index < len(self.R_i):
                        total_gain += self.R_i[index] * 175 + 14000
            ...
            for key, value in x_values.items():
                if value == 1.0:
                    i, j = key.split('_')[1:]
                    if (i, j) in self.D_ij:
                        total_distance += value * self.D_ij[(i, j)]

            solution_csv_filename = os.path.join(self.solution_csv_path, f'solution_{base_name}.csv')
            # with open(solution_csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
            with open(solution_csv_filename, 'w', newline='', encoding='cp949') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Total_Gain', 'Total_distance'])
                writer.writerow([total_gain, total_distance ] )
            print(f"데이터가 {solution_csv_filename} 파일에 저장되었습니다.")

        except Exception as e:
            print("An error occurred:", e)


def solve(datafile, battery_json_path, truck_json_path , drawroute_json_path, solution_csv_path):
    try:
        solver = two_phase_heuristic(datafile, battery_json_path, truck_json_path ,drawroute_json_path,solution_csv_path)
        solver.read_datafile()
        solver.construct_graph()

        print("Solving battery route...")
        solver.simulated_annealing(60)
